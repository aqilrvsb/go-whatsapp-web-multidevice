...
print("    - Failed: 19")
print("    - Remaining: 250")
print("    - Calculation: 158 + 19 + 250 = 427 [OK]")
...
print("    - Failed: 50")
print("    - Remaining: 250")
print("    - Calculation: 207 + 50 + 250 = 507 [OK]")
...
print(f"    - Calculation: {done} + {failed} + {remaining} = {should_send} [OK]")
...
print(f"  [OK] Done Send: {done}")
print(f"  [OK] Failed Send: {failed}")
print(f"  [OK] Remaining Send: {remaining}")
print(f"  [OK] Total Should Send: {should_send}")
...
# Check if maybe summary is using a different user_id filter
print("\n6. ADDITIONAL CHECK - Without user_id filter:")
no_user_query = """
SELECT 
    COUNT(DISTINCT CASE WHEN status = 'sent' AND (error_message IS NULL OR error_message = '') THEN recipient_phone END) AS done_send,
    COUNT(DISTINCT CASE WHEN status = 'failed' THEN recipient_phone END) AS failed_send,
...
cursor.execute(no_user_query, (cold_sequence_id, target_date))
done2, failed2, remaining2 = cursor.fetchone()
should_send2 = done2 + failed2 + remaining2
print(f"  Without user_id filter: Done={done2}, Failed={failed2}, Remaining={remaining2}, Total={should_send2}")
