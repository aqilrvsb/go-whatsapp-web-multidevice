...
def fix_all_duplicate_issues():
    ...
    with open('src/repository/broadcast_repository.go', 'w') as f:
        f.write(content)
    print("   ✓ Added 'processing' status to duplicate checks")
    ...
    with open('src/usecase/campaign_trigger.go', 'w') as f:
        f.write(content)
    print("   ✓ Fixed sequence duplicate check in ProcessDailySequenceMessages")
...
"""
    ...
    with open('add_unique_constraints.sql', 'w') as f:
        f.write(sql_content)
    print("   ✓ Created add_unique_constraints.sql")
    
    print("\n" + "="*50)
    print("FIXES APPLIED:")
    print("="*50)
    print("1. ✓ Added 'processing' status to all duplicate checks")
    print("2. ✓ Fixed sequence duplicate check in campaign_trigger.go")
    print("3. ✓ Created SQL for unique constraints")
    print("\nNEXT STEPS:")
