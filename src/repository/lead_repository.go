...
func (r *leadRepository) GetLeadsByNiche(niche string) ([]models.Lead, error) {
	...
	query := `
		SELECT id, device_id, device_name, user_id, name, phone, niche, journey, status,
	...
	for rows.Next() {
		...
		err := rows.Scan(&lead.ID, &lead.DeviceID, &lead.DeviceName, &lead.UserID, &lead.Name, &lead.Phone,
...
func (r *leadRepository) GetLeadsByDeviceNicheAndStatus(deviceID, niche, status string) ([]models.Lead, error) {
	...
	query := `
		SELECT id, device_id, device_name, user_id, name, phone, niche, journey, status, target_status, ` + "`trigger`" + `, created_at, updated_at
		FROM leads
		WHERE device_id = ?
