...
func (r *BroadcastRepository) QueueMessage(msg domainBroadcast.BroadcastMessage) error {
	...
	// Use provided device_name if available (from leads table)
	if msg.DeviceName != "" {
		deviceName = msg.DeviceName
		if msg.UserID != "" {
			userID = msg.UserID
		} else {
			// Try to fetch user_id if not provided
			err := r.db.QueryRow("SELECT user_id FROM user_devices WHERE device_name = ?", msg.DeviceName).Scan(&userID)
			if err != nil {
				logrus.Warnf("Could not fetch user_id for device_name %s: %v", msg.DeviceName, err)
				userID = ""
			}
		}
	} else if msg.UserID != "" {
		// Fallback: lookup device_name from user_devices
		userID = msg.UserID
