...
func (p *DirectBroadcastProcessor) ProcessDirectEnrollments() (int, error) {
	...
	query := `
		SELECT DISTINCT 
			l.id, l.phone, l.name, l.device_id, l.user_id, l.device_name,
	...
	for rows.Next() {
		...
		if err := rows.Scan(&lead.ID, &lead.Phone, &lead.Name, &lead.DeviceID, &lead.UserID, &lead.DeviceName,
...
func (p *DirectBroadcastProcessor) processCampaignDirect(campaign *models.Campaign) (int, error) {
	...
	enrolledCount := 0
	for rows.Next() {
		var leadID, phone, name, deviceID, userID, deviceName string
		if err := rows.Scan(&leadID, &phone, &name, &deviceID, &userID, &deviceName); err != nil {
			continue
		}
