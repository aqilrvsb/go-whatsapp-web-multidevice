...
// AssignDevicesToTeamMember assigns devices to a team member
func (h *TeamMemberHandlers) AssignDevicesToTeamMember(c *fiber.Ctx) error {
	ctx := context.Background()
	...
	// Get device IDs from request body
	var req struct {
	...
	// Get user ID from context (admin doing the assignment)
	userID := c.Locals("userID").(string)
	adminID, _ := uuid.Parse(userID)
	
	// First, remove all existing assignments
	db := database.GetDB()
	_, err = db.ExecContext(ctx, "DELETE FROM team_member_devices WHERE team_member_id = $1", memberID)
	if err != nil {
		return c.Status(fiber.StatusInternalServerError).JSON(fiber.Map{
	...
	// Then assign new devices
	for _, deviceIDStr := range req.DeviceIDs {
		deviceID, err := uuid.Parse(deviceIDStr)
		if err != nil {
			continue
		}
		h.repo.AssignDeviceToTeamMember(ctx, memberID, deviceID, adminID)
	}
...
// GetTeamMemberWithDevices returns team member details with assigned devices
func (h *TeamMemberHandlers) GetTeamMemberWithDevices(c *fiber.Ctx) error {
	ctx := context.Background()
	...
	deviceIDs, err := h.repo.GetAssignedDeviceIDs(ctx, memberID)
	if err != nil {
		deviceIDs = []string{}
	}
