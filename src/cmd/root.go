...
func initEnvConfig() {
	...
	// Database settings
	// DB_URI is used for WhatsApp (must be PostgreSQL or SQLite)
	if envDBURI := viper.GetString("DB_URI"); envDBURI != "" {
		config.DBURI = envDBURI
	}
	
	// MYSQL_URI is set separately for application data
	// This is handled in database/connection.go
...
func initFlags() {
	...
	rootCmd.PersistentFlags().StringVarP(
		...
		"db-uri", "",
		config.DBURI,
		"the database uri to store the connection data database uri (by default, we'll use sqlite3 under storages/whatsapp.db). database uri --db-uri <string> | example: --db-uri=\"file:storages/whatsapp.db$1_foreign_keys=on or postgres://user:password@localhost:5432/whatsapp\"",
	)
